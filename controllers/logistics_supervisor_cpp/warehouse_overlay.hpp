...
public:
  ...
  void draw(const Snapshot &state) {
    ...
    rectangle(0, 0, kWidth, showWarning ? kHeight : kStatusHeight, kBackground);

    const int delivered = static_cast<int>(std::count_if(
      state.boxes.begin(), state.boxes.end(), [](const Box &box) { return box.delivered; }));
